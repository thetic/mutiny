...
Ordered tests run as a block, separated from regular `TEST` tests. The block of ordered tests runs after all unordered tests by default.

## C Wrapper

`TEST_ORDERED_C_WRAPPER(group, name, level)` wires a C test function into an ordered test. Use it in the `.test.cpp` companion file alongside `TEST_C_WRAPPER`:
